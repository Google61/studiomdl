...
void SimplifyModel (void)
{
	...
	for (j = 0; j < numhitgroups; j++)
	{
		for (k = 0; k < numbones; k++)
		{
			if (strcasecmp( bonetable[k].name, hitgroup[j].name) == 0)
			{
	...
	else
	{
		for (j = 0; j < numhitboxes; j++)
		{
			for (k = 0; k < numbones; k++)
			{
				if (strcasecmp( bonetable[k].name, hitbox[j].name) == 0)
				{
					hitbox[j].bone = k;
					break;
				}
...
int lookup_texture( char *texturename )
{
	...
	else if (stristr( texturename, "fullbright" ) != NULL)
		texture[i].flags |= STUDIO_NF_FLATSHADE | STUDIO_NF_FULLBRIGHT;
	else if (stristr( texturename, "bright" ) != NULL)
		texture[i].flags |= STUDIO_NF_FLATSHADE | STUDIO_NF_FULLBRIGHT;
	else if( stristr( texturename, "_norm" ))
		texture[i].flags = STUDIO_NF_NORMALMAP;
	else if( stristr( texturename, "_gloss" ))
		texture[i].flags = STUDIO_NF_GLOSSMAP;

	if( store_uv_coords && !( texture[i].flags & STUDIO_NF_CHROME ))
		texture[i].flags |= STUDIO_NF_UV_COORDS;
...
void TextureCoordRanges( s_mesh_t *pmesh, s_texture_t *ptexture  )
{
	...
	for( i = 0; i < pmesh->numtris; i++ )
	{
		for( j = 0; j < 3; j++ )
		{
			if( pmesh->triangle[i][j].u > 2.0f ) pmesh->triangle[i][j].u = 2.0f;
			if( pmesh->triangle[i][j].u < -1.0f ) pmesh->triangle[i][j].u = -1.0f;
			if( pmesh->triangle[i][j].v > 2.0f ) pmesh->triangle[i][j].v = 2.0f;
	...
	if (!clip_texcoords)
	{
		for (i=0 ; i<pmesh->numtris ; i++) {
			for (j = 0; j < 3; j++) {
				...
				ptexture->min_t = fmin( pmesh->triangle[i][j].t, ptexture->min_t );
			}
		}
	}
	else
...
// dword alignment for each scan
	...
	if (ptexture->size > 2048 * 2048 + 256 * 3)
	{
		printf("%.0f %.0f %.0f %.0f\n", ptexture->min_s, ptexture->max_s, ptexture->min_t, ptexture->max_t );
...
void ParseScript (void)
{
	while (1)
	{
		...
		else if (!strcasecmp(token, "$rotate"))// XDM
		{
			Cmd_Rotate();
		}
